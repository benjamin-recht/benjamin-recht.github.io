...
$$
\begin{array}{ll}
\mbox{maximize}_{u_t} & \mathbb{E}_{e_t}[ \sum_{t=0}^\infty \gamma^t R[x_t,u_t] ]\\
...
$$
V(x) = \max_u \mathbb{E}_{e_t}\left[R[x,u] + V(f(x,u,e))\right]\,.
$$
...
Note that a perfectly valid expression for the value function is
...
[xxx VIDEO](https://homes.cs.washington.edu/~todorov/media/ErezHumanoids13.mp4)

All these behaviors were generated by MPC in real-time.  The walking is not as what can be obtained from computationally intensive long-horizon trajectory optimization, but it looks considerably better than the sort of direct policy search gaits [we discussed a previous post](xxx).
