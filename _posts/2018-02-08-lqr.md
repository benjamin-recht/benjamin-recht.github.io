...
*This is the fourth part of ["An Outsider's Tour of Reinforcement Learning."](http://www.argmin.net/2018/02/20/outsider-rl/)  Part 5 is [here](http://www.argmin.net/2018/02/14/rl-game/). Part 3 is [here](http://www.argmin.net/2018/02/05/linearization/). Part 1 is [here](http://www.argmin.net/2018/01/29/taxonomy/).*
...
Note that the quadrotor dynamics we [derived from Newton's Laws](http://www.argmin.net/2018/02/01/control-tour/) are linear. The state of the quadrotor is its vertical position and velocity $x_t = [z_t;v_t]$. The input $u_t$ is the propeller force. Newton's Laws written in matrix form are thus
...
So the formula for LQR are a bit gross in terms of all of the matrices and inverses, but the solution should be clear here. But note that this is simple to compute and implement. Indeed, we can design LQR controllers in just a [few lines of python](https://nbviewer.jupyter.org/url/argmin.net/code/little_LQR_demo.ipynb).
