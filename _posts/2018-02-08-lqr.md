...
for some matrix $M_t$. That is, the costate is a linear function of $x_t$. To see this note that by the inductive hypothesis 	$p_{t+1} = M_{t+1} x_{t+1}$. If we combine this identity with the facts that $x_{t+1} = Ax_t + Bu_t$ and $u_t = -R^{-1} p_{t+1}$, we find that
$$
	p_{t+1} = (I+M_{t+1}BR^{-1} B^T )^{-1} M_{t+1} Ax_t
$$

That is $p_{t+1}$ is a linear function of $x_t$. Now, $\nabla_{x_t} \mathcal{L}=0$ implies
$p_{t} = Q x_t + A^T p_{t+1}$, which let's us write
